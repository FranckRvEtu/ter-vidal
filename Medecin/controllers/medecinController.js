...
const loginMedecin = async (req, res) => {
  //belec faudra créer une collection pour les medecins
  ...
  try {
    ...
    if (!isMatch) {
      ...
      const accessToken = jwt.sign(
        //permets de créer un accessToken
        ...
        { expiresIn: "5m" } // durée de vie du token
      );
      const refreshToken = jwt.sign(
        //permets de créer un refreshToken
        { email: medecin.email }, //on 'stocke' l'email du Medecin dans le token
...
const updateMedecin = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la mise à jour du Medecin" });
  }
};
...
const deleteMedecin = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la suppression du Medecin" });
  }
};
