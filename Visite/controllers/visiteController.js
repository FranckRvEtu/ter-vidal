...
const updateVisite = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la mise à jour de la visite" });
  }
};
...
const deleteVisite = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la suppression de la visite" });
  }
};

const daylyVisiteCount = async (req, res) => {
  try {
    const visites = await Visite.find({});
    const today = new Date();
    const todayVisites = visites.filter(
      (visite) => visite.date.getDate() === today.getDate()
    );
    res.json(todayVisites.length);
  } catch (error) {
    console.error(error);
    res
      .status(500)
